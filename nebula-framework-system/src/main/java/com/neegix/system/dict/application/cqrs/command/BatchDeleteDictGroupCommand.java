...
public class BatchDeleteDictGroupCommand implements ICommand<Void> {
    private final List<Long> ids;

    public BatchDeleteDictGroupCommand(List<Long> ids) {
        this.ids = ids;
    }
    @Override
    public Void execute(ApplicationContext context) {
        ...
        transactionTemplate.execute(status -> {
            ...
            dictGroupMapper.deleteByPrimaryKeys(ids);
            // 删除字典组对应的字典项
            DictItemMapper dictItemMapper = context.getBean(DictItemMapper.class);

            NebulaSQL nebulaSQL = new NebulaSQL();
            nebulaSQL.createWhereGroups(DictItemWhereGroup.class).andPkDictGroupIn(ids);
