 ...
 * @Description:
 */
public class NebulaAggregateFunctionSQL{
    ...
    @Setter
    private Pager pager;
    public NebulaAggregateFunctionSQL() {
        whereClauses = new ArrayList<>();
    }

    public NebulaAggregateFunctionSQL setAggregateFunctions(AggregateFunction... aggregateFunction) {
        aggregateFunctions = new ArrayList<>(List.of(aggregateFunction));
        return this;
    }

    public NebulaAggregateFunctionSQL setPager(int currentPage, int pageSize){
        this.pager = new Pager(currentPage, pageSize);
        return this;
    }


    public NebulaAggregateFunctionSQL setGroupByClauses(String... columns) {
        groupByClauses = new ArrayList<>(List.of(columns));
        return this;
    }

    // 添加链式调用的 orderBy 方法
    public NebulaAggregateFunctionSQL orderBy(String column, EnumOrder enumOrder) {
    ...
    // 重载方法，允许链式调用时不指定排序方向
    public NebulaAggregateFunctionSQL orderBy(String column) {
        if (orderByClauses == null) {
            orderByClauses = new ArrayList<>(); // 确保orderByClauses不为null
        }
